...
3. 치우친 방향에 따라 `vesc.yaml`의 값을 수정합니다. 처음엔 **±0.02** 단위로 크게 움직이고, 거의
   똑바로 가기 시작하면 **±0.005** 단위로 좁혀가세요.
   - 왼쪽으로 치우침 → `steering_angle_to_servo_offset` **증가** (예: `0.5304` → `0.5504`)
   - 오른쪽으로 치우침 → `steering_angle_to_servo_offset` **감소** (예: `0.5304` → `0.5104`)
...
4. `/odom`이 보고한 거리와 줄자로 잰 실제 거리를 비교해 게인을 조정합니다. 감(感)으로 조정하는 대신,
   두 거리의 **비율**로 다음 값을 바로 계산할 수 있습니다.

   ```
   새 게인 = 기존 게인 × (실측 거리 ÷ /odom 보고 거리)
   ```

   예를 들어 3.0m 직진을 명령했는데 `/odom`은 2.7m로 보고했고(보고된 거리 < 실제 거리), 줄자로는
   실제로 3.0m를 갔다면:

   ```
   새 게인 = 4614.0 × (3.0 ÷ 2.7) = 4614.0 × 1.111 ≈ 5127.0   (게인 증가)
   ```

   반대로 `/odom`이 3.2m로 보고했다면(보고된 거리 > 실제 거리) `4614.0 × (3.0 ÷ 3.2) ≈ 4326.0`으로
   게인이 줄어듭니다. 처음 몇 번은 이 비율 계산으로 빠르게 근접시키고, 오차가 5% 이내로 줄어들면
   **±100~200** 단위의 미세 조정으로 마무리하세요.
...
3. 실제로 그려진 원의 반경을 측정합니다 — 원본 자료의 기준 차량은 목표값이 **1.722m**였습니다. 이
   ...
   넓어지는지"를 기준으로 삼으세요.
4. 목표 반경과 비교해 게인을 조정합니다. `steering_angle_to_servo_gain`은 **음수**이므로, "게인을
   증가시킨다"는 건 절댓값을 줄이는 방향(0에 더 가깝게)이라는 점에 주의하세요 — 예시 값으로 방향을
   확실히 잡으세요.
   - 목표 반경에 못 미침(원이 너무 좁음) → 게인 **감소**(더 음수로): 예 `-1.2135` → `-1.2635`
   - 목표 반경을 넘어섬(원이 너무 넓음) → 게인 **증가**(덜 음수로): 예 `-1.2135` → `-1.1635`
   - 처음엔 **±0.05** 단위로, 반경이 목표에 가까워지면 **±0.01** 단위로 좁혀가세요.
5. `./rebuild_and_launch.sh`로 재빌드·재실행하고, 반경이 충분히 가까워질 때까지 3~4를 반복합니다.

일반적인 값 범위: **1.1 ~ 1.3**(절댓값 기준 — `vesc.yaml`에는 음수로 저장됩니다).
